...
const emptyForm = { rfqCode: '', transactionDate: new Date().toISOString().slice(0, 10), description: '', quantity: '1', unitCost: '', expenseAccountId: '', supplierId: '' };
...
const TimeExpensesView: React.FC<Props> = ({ orgId, vendors, accounts, currency, currentUserId, onCreatePayable, onNotify }) => {
  ...
  const selectedExpenseAccount = selectedRows[0]?.expenseAccountId;
  ...
  const saveExpense = async (event: React.FormEvent) => {
    ...
    const supplier = vendors.find(vendor => vendor.id === form.supplierId);
    const expenseOrgId = orgId || supplier?.orgId;
    ...
    try {
      ...
      if (editingId) {
        ...
        const saved = await service.createTimeExpense({
          ...values, id: '', claimedBy: 'Not specified', status: 'open', createdBy: currentUserId,
  ...
  const toggle = (row: TimeExpense) => {
    if (selected.length && !selected.includes(row.id) && selectedSupplier !== row.supplierId) {
      onNotify('info', 'One AP Bill can only combine expenses from the same supplier.');
      return;
    }
  ...
  const consolidate = async () => {
    ...
    if (!selectedExpenseAccount) return onNotify('error', 'A selected expense is missing its expense account tag.');
    ...
    try {
      ...
      const bill = await onCreatePayable({
        ...
        expenseAccountId: selectedExpenseAccount, glAccountId: vendor.apAccountId, notes: `Claimed by: ${[...new Set(selectedRows.map(row => row.claimedBy))].join(', ')}`,
