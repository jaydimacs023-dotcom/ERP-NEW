...
const SubscriptionView: React.FC<SubscriptionViewProps> = ({ organization, onUpdate }) => {
  ...
  if (organization.subscriptionStatus === 'PENDING' && currentPlanDetails) {
    return (
      <div className="max-w-full mx-auto py-12 animate-in zoom-in-95 duration-500">
          ...
          <div className="p-12 space-y-10">
             <div className="grid grid-cols-2 gap-8">
                ...
                <div className="space-y-1 text-right">
                   ...
                   <p className="text-lg font-mono font-semibold text-brand">{organization.paymentReference}</p>
                </div>
             </div>
    ...
    return (
      <div className="space-y-8 max-w-4xl mx-auto pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <header className="text-center space-y-3">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-brand/10 border border-brand-light rounded-full text-brand text-xs font-semibold uppercase tracking-wide">
            ...
            <div className="bg-white rounded border border-gray-200 p-8 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-gray-100 flex justify-between items-center">
                  ...
                  <span className="text-xl font-semibold text-brand">${currentPlanDetails.price}</span>
                </div>
              </div>
            </div>
            
            <div className="bg-brand/10 rounded p-6 flex gap-3 border border-brand-light">
            ...
            <div className="grid grid-cols-1 gap-4">
              <button 
                ...
                className={`flex items-center justify-between p-6 rounded-md border-2 transition-all text-left ${
                  paymentMethod === 'GCASH' ? 'border-brand bg-brand/5 shadow-sm shadow-brand/20' : 'border-gray-100 bg-white hover:border-gray-300 shadow-sm'
                }`}
              >
                <div className="flex items-center gap-4">
                  <div className={`p-4 rounded ${paymentMethod === 'GCASH' ? 'bg-brand text-white' : 'bg-gray-50 text-gray-400'}`}>
                    <Smartphone size={24} />
                  </div>
                  <div>
              ...
              <button 
                ...
                className={`flex items-center justify-between p-6 rounded-md border-2 transition-all text-left ${
                  paymentMethod === 'BPI' ? 'border-brand bg-brand/5 shadow-sm shadow-brand/20' : 'border-gray-100 bg-white hover:border-gray-300 shadow-sm'
                }`}
              >
                <div className="flex items-center gap-4">
                  <div className={`p-4 rounded ${paymentMethod === 'BPI' ? 'bg-brand text-white' : 'bg-gray-50 text-gray-400'}`}>
                    <Landmark size={24} />
                  </div>
                  <div>
              ...
              <div className="bg-gray-900 rounded-md p-5 text-white shadow-md animate-in zoom-in-95 duration-300 relative overflow-hidden">
                <div className="relative z-10 space-y-8">
                  <div className="flex justify-between items-center">
                    <h4 className="text-xs font-semibold text-brand uppercase tracking-wide">Institutional Settlement Vault</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-2">
                       ...
                       <div className="flex items-center gap-3">
                          <p className="text-lg font-mono font-semibold text-brand tracking-tighter">{paymentMethod === 'GCASH' ? '0955 231 5522' : '5606-0413-67'}</p>
                  ...
                  <div className="pt-8 border-t border-white/10 space-y-4">
                    <div className="space-y-2">
                       <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide flex items-center gap-2">
                          <Hash size={12} className="text-brand" /> Fund Transfer Reference Number
                       </label>
                       <input 
                          type="text"
                          placeholder="Provide the unique network txn id..."
                          className="w-full bg-white/5 border-2 border-white/10 rounded px-6 py-5 text-lg font-mono font-semibold text-white focus:border-brand outline-none transition-all placeholder:text-gray-800"
  ...
  return (
    <div className="space-y-12 max-w-6xl mx-auto pb-24 animate-in fade-in duration-700">
      <header className="text-center space-y-4">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-brand/10 border border-brand-light rounded-full text-brand text-xs font-semibold uppercase tracking-wide mb-2">
          ...
          return (
            <div 
              ...
              className={`relative bg-white rounded-md border-2 transition-all flex flex-col group ${
                isCurrent ? 'border-brand shadow-md shadow-brand/20 scale-105 z-10' : 'border-gray-100 hover:border-gray-300 shadow-sm'
              }`}
            >
              {plan.isPopular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-brand text-white px-6 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wide shadow-sm shadow-brand/20">
              ...
              <div className="p-5 border-b border-gray-50">
                <div className={`w-16 h-16 rounded-md flex items-center justify-center mb-6 border shadow-sm transition-all group-hover:scale-110 ${
                  plan.id === 'ENTERPRISE' ? 'bg-gray-800 text-brand border-gray-900' :
                  plan.id === 'PROFESSIONAL' ? 'bg-brand/10 text-brand border-brand-light' :
              ...
              <div className="p-5 flex-1 space-y-5">
                {plan.features.map((feature, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className={`mt-0.5 p-0.5 rounded-full ${plan.id === 'BASIC' ? 'bg-emerald-100 text-emerald-600' : 'bg-brand/10 text-brand'}`}>
              ...
              <div className="p-5 pt-0">
                {isCurrent && !isTrial ? (
                  <div className="w-full py-4 rounded border-2 border-brand text-brand text-center text-xs font-semibold uppercase tracking-wide flex items-center justify-center gap-2">
                  ...
                  <button 
                    ...
                    className={`w-full py-4 rounded-md text-xs font-semibold uppercase tracking-wide transition-all active:scale-95 shadow-sm ${
                      ...
                      plan.id === 'PROFESSIONAL' ? 'bg-brand text-white shadow-brand/20 hover:bg-brand-hover' :
      ...
      <div className="bg-white rounded-md p-12 border-2 border-brand shadow-md shadow-brand/20 relative overflow-hidden flex flex-col md:flex-row items-center gap-16">
         <div className="flex-1 space-y-6 relative z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand text-white text-xs font-semibold uppercase tracking-wide rounded-lg">
      ...
      <div className="bg-gray-800 rounded-md p-12 text-white shadow-md flex flex-col md:flex-row items-center gap-12 border border-white/5">
         ...
         <div className="p-8 bg-white/5 rounded-md border border-white/10 grid grid-cols-2 gap-5 shrink-0">
            <div className="text-center">
               <p className="text-xl font-mono font-semibold text-brand">99.9%</p>
