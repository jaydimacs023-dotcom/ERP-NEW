  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">Professional Trainers</h2>
                ...
                <td className="px-6 py-5">
                  <div className="flex flex-wrap gap-1.5">
                    {trainer.qualificationIds.length > 0 ? trainer.qualificationIds.map(id => {
                      ...
                      return qual ? (
                        <div key={id} className="px-2 py-0.5 bg-teal-50 text-teal-700 border border-teal-100 rounded-md text-[9px] font-semibold uppercase tracking-tight">
                          {qual.code}
                        </div>
                      ) : null;
                    }) : (
                    ...
                    <button 
                      ...
                      className="p-1 hover:bg-teal-50 text-teal-400 hover:text-teal-600 rounded transition-colors"
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[70] overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-200 border border-slate-200 my-8">
            <div className="p-6 border-b flex justify-between items-center bg-slate-50/50">
              <div className="flex items-center gap-3">
                <div className={`p-2 ${editingTrainer ? 'bg-amber-600' : 'bg-teal-600'} text-white rounded-xl shadow-md`}>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input required placeholder="Elena" className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-1 focus:ring-teal-600 outline-none text-sm font-medium"
                    value={formData.firstName} onChange={e => setFormData({...formData, firstName: e.target.value})} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest">Middle Name</label>
                  <input placeholder="Santos" className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-1 focus:ring-teal-600 outline-none text-sm font-medium"
                ...
                <div className="space-y-1.5">
                  ...
                  <input required placeholder="Reyes" className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-1 focus:ring-teal-600 outline-none text-sm font-medium"
              ...
              <div className="space-y-1.5">
                ...
                <input required type="email" placeholder="trainer@institution.edu" className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-1 focus:ring-teal-600 outline-none text-sm font-medium"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input placeholder="+63 9xx xxx xxxx" className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-1 focus:ring-teal-600 outline-none text-sm font-medium"
                ...
                <div className="space-y-1.5">
                  ...
                  <input placeholder="e.g. IT, Automotive" className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-1 focus:ring-teal-600 outline-none text-sm font-medium"
              ...
              <div className="pt-6 flex gap-3">
                ...
                <button 
                  type="submit" 
                  disabled={isSubmitting}
                  className={`flex-1 py-3 ${editingTrainer ? 'bg-amber-600 shadow-amber-100' : 'bg-teal-600 shadow-teal-100'} text-white rounded-2xl text-sm font-semibold shadow-md active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2`}
                >
                  {isSubmitting ? (
                    <>
                ...
                return (
                  <button 
                    ...
                    className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all text-left group ${
                      isAssigned ? 'bg-teal-600 border-teal-700 text-white shadow-md shadow-teal-100' : 'bg-white border-slate-100 hover:border-teal-200'
                    }`}
                  >
                    <div className="flex-1">
                      <div className={`text-xs font-semibold uppercase tracking-tight ${isAssigned ? 'text-teal-100' : 'text-teal-600'}`}>{qual.code}</div>
                      <div className={`text-sm font-medium mt-0.5 ${isAssigned ? 'text-white' : 'text-slate-900'}`}>{qual.name}</div>
                    </div>
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${
                      isAssigned ? 'bg-white border-white text-teal-600' : 'bg-slate-50 border-slate-100 text-transparent group-hover:border-teal-200'
                    }`}>
