    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 rounded-3xl p-6 text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl relative overflow-hidden">
         ...
         <div className="flex items-center gap-6 relative z-10">
            <div className="p-4 bg-white/10 rounded-2xl border border-white/10 text-teal-400">
               <Info size={32} />
            </div>
            <div>
         ...
         <div className="shrink-0 relative z-10 px-6 py-2 bg-teal-600/20 border border-teal-500/30 rounded-xl text-[10px] font-black uppercase tracking-widest text-teal-400">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
        <div className="relative w-full sm:w-96">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-1 focus:ring-teal-500 outline-none text-sm transition-all font-bold"
            value={searchTerm}
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-colors shadow-sm bg-slate-50 text-slate-400 border-slate-200 group-hover:bg-teal-600 group-hover:text-white group-hover:border-teal-600`}>
                        <Tag size={20} />
                      </div>
                      <div>
                        <div className="text-sm font-black text-slate-800 leading-tight uppercase">{item.name}</div>
                        <div className="text-[9px] font-mono font-black text-teal-600 uppercase tracking-tighter mt-1">{item.code}</div>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                       <div className={`text-[10px] font-black px-2.5 py-1 rounded-full w-fit uppercase ${item.taxCategoryId === 'VAT' ? 'bg-teal-100 text-teal-700' : 'bg-slate-100 text-slate-500'}`}>
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                       <button onClick={() => { setEditingItem(item); setFormData(item); setShowModal(true); }} className="p-2 hover:bg-teal-50 text-slate-300 hover:text-teal-600 rounded-lg transition-colors"><Edit2 size={16} /></button>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-200 border border-slate-200">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-4">
                  <div className="col-span-1 space-y-1.5">
                    ...
                    <input required placeholder="NCII-01" className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none font-mono text-xs font-black text-teal-600 uppercase"
                ...
                <div className="space-y-1.5">
                  ...
                  <select className="w-full px-4 py-3 bg-white border-2 border-teal-100 rounded-2xl outline-none text-sm font-black text-teal-700 appearance-none"
              ...
              <div className="pt-6 flex gap-3">
                ...
                <button type="submit" className="flex-1 py-3.5 bg-teal-600 text-white rounded-2xl text-sm font-bold shadow-lg shadow-teal-100 active:scale-95 transition-all">Commit Definition</button>
              </div>
            </form>
          </div>
        </div>
