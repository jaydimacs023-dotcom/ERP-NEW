...
import { ChartOfAccount, InventoryClass, WarehouseLocation } from '../types';
...
export default function InventoryClassesView({ classes, accounts, warehouses, onSave }: Props) {
  ...
  const accountField = (label: string, key: keyof InventoryClass, required = false) => (
    <label className="space-y-1.5">
      ...
      <select
        ...
        className="w-full h-11 px-3 rounded border border-gray-200 bg-white text-sm"
      >
  ...
  return (
    <div className="space-y-6 pb-20">
      <header className="flex items-end justify-between gap-4">
        ...
        {!editing && (
          <button onClick={() => setEditing(emptyClass)} className="flex items-center gap-2 px-5 py-3 bg-brand text-white rounded text-xs font-semibold uppercase">
        ...
        <form onSubmit={save} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-5">
            <label className="space-y-1.5">
              ...
              <input required value={editing.code || ''} onChange={event => setEditing({ ...editing, code: event.target.value.toUpperCase() })} className="w-full h-11 px-3 rounded border" />
            </label>
            <label className="space-y-1.5 md:col-span-2">
              <span className="text-xs font-semibold uppercase text-gray-500">Name *</span>
              <input required value={editing.name || ''} onChange={event => setEditing({ ...editing, name: event.target.value })} className="w-full h-11 px-3 rounded border" />
            </label>
            ...
            <label className="space-y-1.5">
              ...
              <select value={editing.defaultWarehouseId || ''} onChange={event => setEditing({ ...editing, defaultWarehouseId: event.target.value })} className="w-full h-11 px-3 rounded border">
            ...
            <label className="space-y-1.5">
              ...
              <select value={editing.valuationMethod} onChange={event => setEditing({ ...editing, valuationMethod: event.target.value as InventoryClass['valuationMethod'] })} className="w-full h-11 px-3 rounded border">
          ...
          <div className="px-6 py-4 border-t flex justify-end">
            <button disabled={saving} className="px-6 py-3 bg-brand text-white rounded text-xs font-semibold uppercase disabled:opacity-50">
      ...
      <div className="bg-white border rounded-xl overflow-hidden">
        <table className="w-full">
          <thead className="bg-brand text-white"><tr><th className="px-4 py-3 text-left">Class</th><th className="px-4 py-3 text-left">Valuation</th><th className="px-4 py-3 text-left">Status</th><th className="px-4 py-3 text-right">Action</th></tr></thead>
          <tbody className="divide-y">
            {classes.map(value => (
              <tr key={value.id}>
                ...
                <td className="px-4 py-3 text-right"><button onClick={() => setEditing(value)} className="p-2 text-gray-500 hover:text-brand"><Edit2 size={16}/></button></td>
              </tr>
            ))}
