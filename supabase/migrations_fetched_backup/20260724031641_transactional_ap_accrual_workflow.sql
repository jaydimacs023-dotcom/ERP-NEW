...
begin
  ...
  for v_i in 1..array_length(p_payable_ids,1) loop
    ...
    v_outstanding := round(coalesce(nullif(v_bill.net_payable,0),v_bill.amount) - coalesce(v_bill.paid_amount,0),2);
  ...
  for v_i in 1..array_length(p_payable_ids,1) loop
    ...
    update public.payables set paid_amount=round(coalesce(paid_amount,0)+v_amount,2), payment_date=p_payment_date,
      ...
      status=case when coalesce(nullif(net_payable,0),amount)-round(coalesce(paid_amount,0)+v_amount,2)<=0.005 then 'paid' else 'partially_paid' end,
      paid_at=case when coalesce(nullif(net_payable,0),amount)-round(coalesce(paid_amount,0)+v_amount,2)<=0.005 then now() else paid_at end
    where id=v_bill.id;
  end loop;
  insert into public.journal_lines (id,journal_entry_id,account_id,debit,credit,description)
