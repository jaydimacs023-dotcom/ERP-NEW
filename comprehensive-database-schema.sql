-- =====================================================
-- AT-ERP COMPREHENSIVE DATABASE SCHEMA
...
CREATE TABLE journal_entry_lines (
    ...
    memo TEXT,
    contact_id UUID, -- Polymorphic reference
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CHECK (debit > 0 OR credit > 0) -- Must have either debit or credit
);
...
CREATE TABLE students (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    org_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    uli VARCHAR(50) UNIQUE NOT NULL, -- Unique Learner Identifier
...
CREATE TABLE qualifications (
    ...
    code VARCHAR(20) NOT NULL,
    name VARCHAR(255) NOT NULL,
    duration_days INTEGER NOT NULL, -- Based on 1 day = 8 hours rule
    sector VARCHAR(100),
    description TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- Schedule time slots (JSON structure alternative)
...
CREATE TABLE sponsorships (
    ...
    student_id UUID NOT NULL REFERENCES students(id) ON DELETE CASCADE,
    batch_id UUID REFERENCES batches(id) ON DELETE SET NULL,
    sponsorship_type VARCHAR(50) NOT NULL, -- 'FULL', 'PARTIAL', 'STIPEND'
    amount DECIMAL(15,2),
    coverage_details JSONB, -- Flexible coverage specification
    start_date DATE NOT NULL,
    end_date DATE,
...
-- All other tables follow similar pattern for organization-based access
...
CREATE POLICY "Organization data access" ON journal_entries
    FOR ALL USING (org_id IN (
        SELECT org_id FROM users WHERE email = auth.email()
    ));

CREATE POLICY "Organization data access" ON journal_entry_lines
...
-- Continue similar policies for all tables...
-- (For brevity, showing pattern - implement for all tables)
...
-- Add similar triggers for all relevant tables...

-- =====================================================
-- VIEWS FOR COMMON QUERIES
-- =====================================================

-- Student with documents view
...
SELECT 
    ...
    COUNT(be.student_id) as enrolled_students
FROM batches b
LEFT JOIN qualifications q ON b.qualification_id = q.id
LEFT JOIN trainers t ON b.trainer_id = t.id
LEFT JOIN locations l ON b.location_id = l.id
LEFT JOIN sponsors sp ON b.sponsor_id = sp.id
LEFT JOIN batch_enrollments be ON b.id = be.batch_id
GROUP BY b.id, q.name, t.first_name, t.last_name, l.name, sp.name;
...
-- 5. Create data migration scripts from existing system
