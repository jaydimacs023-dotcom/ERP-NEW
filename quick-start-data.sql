...
-- Create default organization (only if not exists)
...
SELECT 
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM organizations WHERE name = 'AccounTech Platform Host'
);
...
SELECT 
    ...
    'admin@accountech.io',
    crypt('admin123', 'salt'), -- Simple salt for now
    'salt',
    'SYSTEM_ADMIN',
    true,
    NOW(),
...
AND NOT EXISTS (
    SELECT 1 FROM users WHERE email = 'admin@accountech.io'
)
LIMIT 1;
...
SELECT 
    ...
    unnest(ARRAY['ASSETS', 'BDO Checking Account', 'Petty Cash', 'Accounts Receivable', 'LIABILITIES', 'Accounts Payable', 'EQUITY', 'Retained Earnings', 'Owners Capital', 'REVENUE', 'Training Revenue', 'EXPENSES', 'Depreciation Expense']),
